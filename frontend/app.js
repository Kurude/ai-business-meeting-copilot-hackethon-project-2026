...
// Segments are recorded back-to-back, but network requests can complete out of
// order. Chaining every upload onto this promise guarantees they land - and get
// appended to the transcript - in the same order they were recorded.
let liveUploadQueue = Promise.resolve();
...
function appendMessage(container, role, content, sources = []) {
  ...
  if (empty && container.id === "chat-window") empty.classList.add("hidden");
...
// ===================== VOICE (now part of the same chat) =====================
el("record-btn").addEventListener("click", async () => {
  ...
  if (!MEDIA_RECORDER || MEDIA_RECORDER.state === "inactive") {
    try {
      ...
      status.textContent = "Listening… tap the mic again to stop";
...
async function handleVoiceStop() {
  ...
  try {
    ...
    appendMessage(chatWindow, "user", data.transcript);
    const replyBubble = appendMessage(chatWindow, "assistant", data.reply);
    const audio = document.createElement("audio");
    audio.controls = true;
    audio.className = "msg-audio";
    audio.src = `${API_BASE}${data.audio_url}`;
    replyBubble.appendChild(audio);
    audio.play();
...
function recordLiveSegment() {
  ...
  liveSegmentRecorder.onstop = async () => {
    ...
    // Chain onto the upload queue so segments are sent - and appended to the
    // transcript - strictly in recording order, even if a request is slow.
    // Recording of the next segment starts immediately regardless.
    liveUploadQueue = liveUploadQueue.then(() => uploadLiveSegment(blob));
...
el("live-end-btn").addEventListener("click", async () => {
  ...
  if (liveSegmentRecorder && liveSegmentRecorder.state !== "inactive") {
    liveSegmentRecorder.stop(); // queues the final in-progress segment for upload
  }
  if (liveStream) {
    liveStream.getTracks().forEach((t) => t.stop());
  }

  // Wait for every queued segment (including the final one just triggered by
  // stop()) to actually finish uploading, rather than guessing with a fixed
  // delay - onstop fires asynchronously so give it a tick to enqueue first.
  await new Promise((resolve) => setTimeout(resolve, 50));
  await liveUploadQueue;
