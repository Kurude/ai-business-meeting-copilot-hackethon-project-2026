...
Mandatory features implemented:
  ...
  - RAG over uploaded PDFs / meeting notes (ChromaDB + built-in ONNX embeddings)
  - AI Chatbot (RAG-grounded)
  - Voice Assistant (STT via Gemini native audio understanding, TTS via gTTS)
...
def _warmup_embedding_model():
    """Trigger the one-time ONNX embedding model download/load at server
    startup rather than on a real user's first chat/document request - avoids
    a slow or failing first request right after a cold start / free-tier wake-up."""
    try:
        rag.warmup()
    except Exception as e:
        print(f"[warmup] embedding model warmup failed (will retry on first real request): {e}")
...
app.add_middleware(
    ...
    # CORS spec (browsers reject it). This app authenticates via a Bearer token header,
    # not cookies, so credentialed (cookie) requests are never needed.
...
def _save_upload(file: UploadFile, save_path: str, max_bytes: int = MAX_UPLOAD_BYTES) -> None:
    """Stream an UploadFile to disk in chunks, rejecting anything over max_bytes
...
def register(user_in: schemas.UserCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Handles the rare race where two near-simultaneous requests (e.g. a
        # double-clicked submit button) both pass the check above before
...
# ============================================================
# frontend/ lives inside this same backend/ folder (the repo root on Render),
# as a sibling of main.py.
_candidate_dirs = [
